...
void readdata(void) {
	FILE* playerfile = fopen(flag.core.playerpath, "r");

	if (playerfile == NULL) {
		log_error("Could not open '%s' for reading.", flag.core.playerpath);
		log_nl(   "Maybe there is no previous player.");
		*game.playername = '\0';
	} else {
		log_debug("Opened '%s' for reading succesfully.", flag.core.playerpath);
		size_t read = fread(game.playername, sizeof(char), length.game.maxnicknamelen, playerfile);
		log_debug("Read %d/%d bytes from '%s'.", read, length.game.maxnicknamelen, flag.core.playerpath);
		fclose(playerfile);
	}
	...
	FILE* scores = fopen(flag.core.scorespath, "r");

	if (scores == NULL) {
		log_error("Could not open '%s' for reading.", flag.core.scorespath);
		log_nl(   "Maybe there is no saved scoreboard.");
		flag.core.score = false;
	} else {
		log_debug("Opened '%s' for reading sucessfully.", flag.core.scorespath);
		int read = 0;
		int i, j;
		...
		log_debug("Read %d bytes from '%s'.", read, flag.core.scorespath);
		fclose(scores);
	}
...
void writedata(void) {
	// save player name
	FILE* playerfile = fopen(flag.core.playerpath, "w+");
	if (playerfile == NULL) {
		log_error("Could not open '%s' for writing.", flag.core.playerpath);
	} else {
		log_error("Opened '%s' for writing successfully.", flag.core.playerpath);
		int written;
		written = fwrite(game.playername, sizeof(char), length.game.maxnicknamelen, playerfile);
		log_debug("The %d/%d bytes has been written to '%s'.", written, length.game.maxnicknamelen, flag.core.playerpath);
		fclose(playerfile);
	}

	// save score
	FILE* scores = fopen(flag.core.scorespath, "w");
	if (scores == NULL) {
		log_error("Could not open '%s' for writting.", flag.core.scorespath);
	} else {
		log_debug("Opened '%s' for writting sucessfully.", flag.core.scorespath);
		int written = 0;
		...
		log_debug("Written %d bytes to '%s'.", written, flag.core.scorespath);
		fclose(scores);
	}
}
