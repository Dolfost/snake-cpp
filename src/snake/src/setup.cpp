...
void setup(void) {
	...
	if (flag.option.nologcolor == false) {
		...
		init_pair(1, COLOR_WHITE, g_log.background);
		for (int i = 0; i <= 5; i++) {
			g_log.levelcolor[i] = 1;
			g_log.levelattr[i]  = A_NORMAL;
		}
		g_log.filenamecolor = 1;
		g_log.msgcolor = 1;
		g_log.filenameattribute = A_NORMAL;
		g_log.msgattribute = A_NORMAL;
	}
	...
	if (helpfile == NULL) {
		...
		log_info("Trying to build help pad at '%s'", flag.option.helppadpath);
		buildhelppad(flag.option.helppadpath);
	} 

	helpfile = fopen(flag.option.helppadpath, "r");
	if (helpfile == NULL) {
		log_error("Could not open help pad file");
		log_nl("'%s' for reading.", flag.option.helppadpath);
		log_nl("Maybe it failed to build.");
	} else {
